        ...
        <div className="flex flex-wrap -m-4">
          {services.map((service, index) => (
            <div className="p-4 lg:w-1/4 md:w-1/2" key={index} data-aos="fade-up">
              <div className="cardss h-full flex flex-col items-center text-center">
                <img alt={service.title} className="rounded-lg w-full h-72 object-cover object-center mb-4" src={service.imgSrc} /> {/* Increased height */}
                
                <div className="content w-full">
                  <h3 className="text-white mb-2">{service.title}</h3>
                  <p className="text-gray-300 mb-3">{service.description}</p>
                  <button className="text-indigo-400 underline">Learn More</button> {/* Styled button */}
                </div>
